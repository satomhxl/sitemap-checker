...
```text
Every day, open the sitemap-checker project on this Mac Mini. Pull the latest code, run `python3 run_daily_report.py --site mediaio --site pincel --since-hours 24`, then commit and push changed files under `reports/` only if there are actual changes. If the command fails, send me the error summary in Telegram. Do not delete `sitemaps.db`.
```
...
```bash
python3 run_daily_report.py --site mediaio --site pincel --since-hours 24
python3 export_events.py --site mediaio --site pincel --output public/data/events.json
```
...
```bash
python3 checker.py --site mediaio --site pincel --show 5
```
